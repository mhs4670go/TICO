...
# by comparing INT vs FP *outputs at each layer*.
#
...
#   4. Calibrate INT-8 activation observers in a single pass.
#   5. Freeze quantization parameters (scale, zero-point).
#   6. Re-run inference and compare INT-8 vs FP outputs per layer.
...
# Token-budget presets for calibration / evaluation.
TOKENS: dict[str, int] = {
    # < 1 min turnaround on CPU/GPU
    "debug": 2_000,
    # Good default for 1-7 B models (≲3 % ppl delta)
    "baseline": 50_000,
    # Production setting (for INT-4 / observer smoothing, etc.)
    "production": 200_000,
}
...
# -------------------------------------------------------------------------
# 2. Wrap every layer with PTQWrapper (INT-8 activations)
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# 3. Activation calibration *plus* FP-vs-INT8 diffing
# -------------------------------------------------------------------------

...
#   • Full post-training INT-8 flow (wrap → calibrate → eval).
# =============================================================================
...
RUN_FP = True  # set False → run INT-8 path
...
if RUN_FP:
    ...
    for idx, layer in enumerate(int8_model.model.layers):
        layer_cfg = qcfg.child(f"layer{idx}")
        wrapped_layers.append(PTQWrapper(layer, qcfg=layer_cfg))
    int8_model.model.layers = wrapped_layers
...
if RUN_FP:
    ...
    ppl_int8 = perplexity(int8_model, enc, DEVICE, stride=STRIDE)

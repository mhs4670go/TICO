...
class QuantFairseqEncoderLayer(QuantModuleBase):
    ...
    - LayerNorms remain in FP; only attention and FFN linear ops are PTQ-wrapped.
    ...
    def __init__(
        ...
        *,
        ...
        # LayerNorms stay FP (reuse original instances to keep weights)
        ...
        # Activation function (reuse; kept in FP)
    ...
    # ----------------------------------------------------------------------
    # Only recurse into children; no additional per-tensor observers here.
    def _all_observers(self):
        yield from (self.obs_activation_fn,)
        for m in (self.self_attn, self.fc1, self.fc2):

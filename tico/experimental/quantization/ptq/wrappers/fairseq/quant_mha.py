...
class QuantFairseqMultiheadAttention(QuantModuleBase):
    ...
    def __init__(
        ...
        *,
        ...
        self.dropout_p: float = float(
            getattr(getattr(fp_attn, "dropout_module", None), "p", 0.0)
        )
    ...
    def forward(
        ...
        if state is not None and "prev_key" in state and static_kv:
            ...
            v = None
        if k is not None:
            k = self._fq(self._fold_heads(k, B), self.obs_k_fold)
        if v is not None:
            v = self._fq(self._fold_heads(v, B), self.obs_v_fold)
        ...
        #     attn_probs = F.dropout(attn_probs, p=self.dropout_p, training=True)
